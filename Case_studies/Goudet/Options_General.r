...
#****************************************
station.name         = "La Loire River at Goudet, France"  # Insert general info of the station (for plots)   
data.period          = "01/2018 - 06/2022"                 # Insert the study period (for plots)


# Gaugings:
#**********
file_gaugings        = "Goudet_gauging_2018_2022.csv" # name of the csv file with the gaugings (put FALSE if not available!)
...
grid_RC.xlim         = c(0,5)                    # grid for RC plots (stage h limits)
grid_RC.ylim         = c(0,300)                   # grid for RC plots (discharge Q limits)
grid_RC.xstep        = 1                          # grid for RC plots 
grid_RC.ystep        = 20                         # grid for RC plots
grid_RC.ylim.log     = c(0.5, 1000)              # grid for RC plots in y-log scale
ticks_RC.y.log       = c(0.5,1,10,100,1000) # grid for RC-log plots
...
# Stage record:
#**************
file_limni           = "Limnigramme_goudet_2018_2022.csv"              # name of the data .csv file with the stage record (put FALSE if not available!)
...
grid_limni.ylim      = c(0,5,1)                          # grid for Limni plots
...
#***********************************************
official.shift.times = "DateShiftsGoudet.csv"  # name of the csv file with official shift dates (put FALSE if not available!)
...
M[2,]             = c(0,1) # main control channel (rectangular wide channel in uniform condition)
...
Cr.prior  = c(0.4, NA);       st_Cr.prior = c(0.05, NA)      # flux coefficient
g.prior   = c(9.81, NA);      st_g.prior  = c(0.005, NA)     # gravity
Bw.prior  = c(6, NA);         st_Bw.prior = c(0.75, NA)        # weir width
#if rectangular channel: a = Ks*S0*Bc
Bc.prior  = c(NA, 25);        st_Bc.prior = c(NA, 3)         # channel width
KS.prior  = c(NA, 15);        st_KS.prior = c(NA, 2.5)         # Strickler coeff.
S0.prior  = c(NA, 0.007);  st_S0.prior = c(NA, 0.0025) # Longitudinal slope
...
b.prior  = c(0.65, 1.1);    st_b.prior = c(0.075, 0.2)


#Parameter c:
#------------
c.distr = "Gaussian"  #"Fix" or "Gaussian" or "Uniform" or "Flat"
c.prior = c(1.5, 1.67);     st_c.prior= c(0.025, 0.025)
