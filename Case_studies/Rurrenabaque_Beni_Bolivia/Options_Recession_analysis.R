...
rec.model                = c("2expWithAsympt")
prior.param.rec[[1]]  = c(  0,                           500,       "'Uniform'",     200,                      "var",
                            -log((5))+log(log(2)) ,      0.5,      "'LogNormal'",   exp(-log(5)+log(log(2))), "static",
                            0,                            100,       "'Uniform'",     50,                         "static",
                            -log(80)+log(log(2)) ,        0.05,      "'LogNormal'",   exp(-log(80)+log(log(2))),  "static",
                            -150                       , 100 ,      "'Uniform'",  0 ,                         "var")
...
#                           -150                 ,        150 ,      "'Uniform'",     0 ,                         "var")  #  this last line (asymptote prior) is specific to your case study !!!!



# rec.model                = c("2expWithAsympt_bis")           # Select stage-recession models (also more than one) (see the list at the bottom):
# prior.param.rec[[1]]  =c( 0,                            1200,       "'Uniform'",     100,                      "var",  # prior: [in cm for param a and days-1 for param b]
#                           -log((5))+log(log(2)) ,        1,      "'LogNormal'",     exp(-log(5)+log(log(2))), "static", # min or mean, max or stdev, distribution, starting value, "static"/"var";
#                           0,                            200,       "'Uniform'",     50,                         "var",
#                           -log(80)+log(log(2)) ,        0.1,       "'LogNormal'",   exp(-log(80)+log(log(2))),  "static",
#                           -150                 ,        300 ,      "'Uniform'",     0 ,                         "var")  #  this last line (asymptote prior) is specific to your case study !!!!


# rec.model                = c("expexp_bis")           # Select stage-recession models (also more than one) (see the list at the bottom):
...
#####################################
prior.mu.rec.segment           = c("Uniform", -150, 100, 0)        # c(distribution,  min or mean,  max or stdev, stanrting value)
gamma.prior.rec                = c("Uniform", 0, 5, 1)         # c(distribution,  min or mean,  max or stdev,  starting value)
nSmax.rec                      = 12                                # Maximum number of change points that can be detected 
criterion.rec                  = "DIC"                            # crtierion for the selection of optimal number of change points ("DIC", "BIC")
seg.plot.results.only          = FALSE                            # [FALSE/TRUE] if TRUE no computation is performed. only results are plotted.
Ncycle.rec.segment             = 1000                             # number of cycles mcmc.
Nmcmc.rec.segment              = 100                              # number of mcmc per cycle.
Nslim.rec.segment              = 50                              # for summary statistics, we consider one mcmc each "Nslim.rec.segment".
