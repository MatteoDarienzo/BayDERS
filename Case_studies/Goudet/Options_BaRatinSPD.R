...
# General options:
################################################################
name.folder.results.SPD  =  "SPD_test"
...
local.change             = TRUE                         # [TRUE/FALSE] this will consider a change of RC parameter "b1" (translation of the lowest control only). 
width.change             = TRUE                         # [TRUE/FALSE] this will consider a change of RC parameter "a" (due to channel width variation). 
n.parvar                 = 3                             # number of dynamic parameters
isVar                    = c(T,T,F, T,F,F)        # [T/F] Define which RC parameters are varying (b1,a1,c1, b2,a2,c2, ...)
dg.prior                 = c(0, 0.35)                    # mean and stdev for incremental global change priors
dl.prior                 = c(0, 0.35)                    # mean and stdev for incremental local change priors
dB.prior                 = c(1, 1)                     # mean and stdev for channel width change priors
b.prior                  = c(0.43, 1.15)                  # mean for parameters "b" for the oldest period 
st_b.prior               = c(0.5,0.5)              # stdev for parameters "b" for the oldest period
param.var                = c("b1","a1","b2")                   # names of parameters that vary (e.g. = c("b1", "b2) )
...
ylim.log.wind            = c(0.08, grid_RC.ylim.log[2])  # define Q limits for plots in log scale (e.g. grid_RC.ylim.log).
