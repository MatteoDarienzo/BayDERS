...
prior.param.rec[[1]]  =c( 0,                            500,       "'Uniform'",     100,                      "var",  # prior: [in cm for param a and days-1 for param b]
                          ...
                          0                 ,           300 ,      "'Uniform'",     100 ,                         "var")  #  this last line (asymptote prior) is specific to your case study !!!!
prior.gamma.rec          = c(0, 2, "'Uniform'",  1,      # Structural error prior gamma1 (intercept):  min or mean, max or stdev, distribution, starting value
                             0, 10,  "'Uniform'", 0.1)    # Structural error prior gamma2 (coeff ang.):  min or mean, max or stdev, distribution, starting value
...
#####################################
seg.plot.results.only          = TRUE                            # [FALSE/TRUE] if TRUE no computation is performed. only results are plotted.
prior.mu.rec.segment           = c("Uniform", 0, 200, 0)          # c(distribution,  min or mean,  max or stdev, stanrting value)
gamma.prior.rec                = c("Uniform", 0, 50, 0.1)         # c(distribution,  min or mean,  max or stdev,  starting value)
nSmax.rec                      = 4                                # Maximum number of change points that can be detected 
...
shift.time.adjustment.type.rec = 3                                # 1 = select always the MAP; 2= select always the largest closest flood peak; 3 = will ask to manually insert a value 
