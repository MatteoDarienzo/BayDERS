# General options:
################################################################
name.folder.results        = "mu2_BIC_3_inc_10"
prior.mu.segm              = c(0, 2)                   # prior for segments mean "mu" = N(mean, stdev): stdev must be of the Order of gaugings residuals !
tmin                       = 0                          # mininum distance (in time) between two consecutive change points (it depends on time units!). By default =0
nSmax                      = 3                          # Maximum number of segments in the series at each iteration
...
st_b.prior                 = c(0.035, 0.5, 0.2,0.2)     # Initial stdev of b:
