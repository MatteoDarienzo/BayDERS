...
# Organisation : INRAE
# Date of last update : 04/09/2022
# Contributors : Felipe Mendez (INRAE Lyon).
#
...
##########################################################################################################
  message('####################################################')
  message('#    Procedure for stage-recessions extraction     #')
  message('####################################################')
  message('Start stage recessions analysis.')
  message('Method described in Darienzo, 2021 (PhD thesis)')
  message('This step performs the extraction of recessions from')
  message('the stage record according to user-defined options.\n')
  ...
  message("- Reading input settings and data ...")
  ...
  BayesianOption         = 2                                   # 2 = Bayesian Pooling method
  toll                   = 0.1                                 # [cm] tolerance for the recession extraction. 
  if (!exists("limni_filter.rec")){                            # filter for the stage record to reduce number of data.
    limni_filter.rec =limni_filter
  } else {
     if (is.null(limni_filter.rec)){
        limni_filter.rec = limni_filter
     }
  }
  ...
  file.options = paste0(dir.extraction,"/options_recession_extraction.txt")
  message(paste0("- Saving user-specified options to file: "))
  print(file.options)
  ...
  cat(paste0("limni_filter.rec  =", limni_filter.rec,  "           # Filter for stage record"),                                         file = file.options, append = TRUE, sep="\n")   
  cat(paste0("gradient.max  =",     gradient.max,  "              # max gradient dh/dt for the recession selection"),                   file = file.options, append = TRUE, sep="\n") 
  # screening selected options for the current test:
  message("- Selected options:")
  print(paste0("  uh.rec = ",            uh.rec,            "               # stdev of the stage obs error [in cm]"))
  print(paste0("  tburn.rec = ",         tburn.rec,         "            # discard the first n days of the studied recession curve"))      
  print(paste0("  Nmin.rec = ",          Nmin.rec,          "               # min number of data in a recession curve"))
  print(paste0("  tgood = ",             tgood,             "               # min length of the recession in days"))   
  print(paste0("  delta.t.min = ",       delta.t.min,       "            # min days between two recess data"))       
  print(paste0("  delta.t.max = ",       delta.t.max,       "           # max days between two recess data"))  
  print(paste0("  chi = ",               chi,               "                   # max stage rise/jump between two recess data"))
  print(paste0("  limni_filter.rec  = ", limni_filter.rec,  "       # Filter for stage record"))   
  print(paste0("  gradient.max  = ",     gradient.max,  "       # max gradient dh/dt for the recession selection"))   


  
  #*********************************************************************************************************
  # filter stage record by user specifications (limni_filter.rec):
  stage.record = stage.record[seq(1, length(stage.record[,1]), limni_filter.rec), ] 
  ...
  if (stage.limni.u.m. == "cm"){
      ...
      print("You have inserted a wrong  u.m. for the stage record! available options are 'm' or 'cm' or 'mm'.")
  }
  ...
  #*******************************************************************************************************************
  message('- Total number of available stage data:')
  ...
  # detect all time final of the recession curves:
    ...
    message('- Saving intermediate results of recession extraction to file:')
    print(paste0(dir.extraction,"/Extract_rec_curves.csv"))
    ...
    ncurves.h    = icurve.h
    message("\n******************************************************************")
    message('- CYCLE on all recessions to filter only the recessions with:')
...
#########################################################################################################
  ...
  ###################################
            ...
            #######################################
            # save selected options to file:
            file.options = paste0(dir.rec.pool.test,"/options_recession_estimation.txt")
            message(paste0("- Saving user-specified options to file: "))
            print(file.options)
            cat(paste0("stage.scale.shift = ", stage.scale.shift),          file = file.options, append = FALSE, sep="\n")
            cat(paste0("recession model = ",   rec.mod),                    file = file.options, append = TRUE, sep="\n") # recession model: "exp", "expexp", "hyperb", etc.
            ...
            cat(paste0("nburn.rec = ",          nburn.rec),                  file = file.options, append = TRUE, sep="\n")
            cat(paste0("prior.param.rec = ",    paste(prior.rec[1:5], collapse=" ")),  file = file.options, append = TRUE, sep="\n")
            for (j in 2:(length(prior.rec)/5)){
                cat(paste0("                   ", paste(prior.rec[(j*5-4):(j*5)], collapse = " ")),  file = file.options, append = TRUE, sep="\n") 
            }
            cat(paste0("gamma.model.rec = ",   gamma.model.rec),            file = file.options, append = TRUE, sep="\n")
            if (gamma.model.rec == "linear"){
              cat(paste0("prior.gamma.rec = ",   paste(prior.gamma.rec[1:4], collapse=" ")),  file = file.options, append = TRUE, sep="\n")
              cat(paste0("                  ",   paste(prior.gamma.rec[5:8], collapse=" ")),  file = file.options, append = TRUE, sep="\n")
            } else if (gamma.model.rec=='constant'){
              cat(paste0("prior.gamma.rec =",   paste(prior.gamma.rec[1:4], collapse=" ")),  file = file.options, append = TRUE, sep="\n")
            }
            
            # screening selected options for the current test:
            message("- Selected options:")
            print(paste0("  stage.scale.shift = ",       stage.scale.shift, "                        #  parameter used to shift stage values and avoid negative values [cm]:"))
            print(paste0("  recession model = ",         rec.mod))      
            print(paste0("  jump.pos.rec / jump.neg.rec = ",   paste(jump.pos.rec, jump.neg.rec, sep=" / "), "          # parameter positive /negative jump rate during mcmc sim."))
            print(paste0("  Ncycles.mcmc.rec = ",             Ncycles.mcmc.rec, "                          # number of mcmc cycles during the bayesian inference for recessions estimation."))   
            print(paste0("  nmcmc.rec = ",       nmcmc.rec,  "                                # number of mcmc per cycle."))       
            print(paste0("  nslim.rec = ",       nslim.rec,  "                                 # number of slimmed mcmc for the summary statistics."))
            print(paste0("  nburn.rec = ",       nburn.rec,  "                                # fraction of initial mcmc burned."))
            print(paste0("  chi = ",               chi,      "                                   # max stage rise/jump between two recess data"))
            print(paste0("  prior.param.rec = ",    paste(prior.rec[1:5], collapse=" "), "       # Priors for the recession model parameters:"))
            for (j in 2:(length(prior.rec)/5)){
                 print(paste0("                    ", paste(prior.rec[(j*5-4):(j*5)], collapse = " "),  " ")) 
            }
            print(paste0("  gamma.model.rec  = ", gamma.model.rec,  "                    # Remnant error model (err = gamma1 + gamma2*h = linear  or  err = gamma1 =constant)."))
            if (gamma.model.rec == "linear"){
              print(paste0("  prior.gamma.rec = ",   paste(prior.gamma.rec[1:4], collapse=" "), "         # Structural error prior"))
              print(paste0("                    ",   paste(prior.gamma.rec[5:8], collapse=" "), " "))
            } else if (gamma.model.rec=='constant'){
              print(paste0("  prior.gamma.rec =",   paste(prior.gamma.rec[1:4], collapse=" "),  "         # Structural error prior")) 
            }
            ...
            # Launch BaM.exe (Benjamin Renard, INRAE)
            # setwd(dir.exe)
            if (Sys.info()['sysname'] == "Linux"){
               system2("./BaM")
            } else if (Sys.info()['sysname'] == "Windows"){
               system2("BaM_2exp_pool2.exe")
            }
            ...
            } else if (rec.mod == "hyperb_bis"){  # a1(k), b1(k), n1, a2(k)
                a1.mcmc   = Results_mcmc.pool[, which.recession]
                b1.mcmc   = Results_mcmc.pool[, Ncurves.pool + which.recession]
                n1.mcmc   = Results_mcmc.pool[, 2*Ncurves.pool + 1]
                asym.mcmc = Results_mcmc.pool[, 2*Ncurves.pool + 1 + which.recession]
                g1.mcmc   = Results_mcmc.pool[, 2*Ncurves.pool + 1 + Ncurves.pool +1]
                if (gamma.model.rec =='linear'){
                  g2.mcmc   = Results_mcmc.pool[, 2*Ncurves.pool + 1 + Ncurves.pool +2]
            ...
            } else if (rec.mod == "Coutagne_bis"){   # a1(k), b1(k), n1, a2(k)
                a1.mcmc   = Results_mcmc.pool[, which.recession]
                b1.mcmc   = Results_mcmc.pool[, Ncurves.pool + which.recession]
                n1.mcmc   = Results_mcmc.pool[, 2*Ncurves.pool + 1]
                asym.mcmc = Results_mcmc.pool[, 2*Ncurves.pool + 1 + which.recession]
...
# stage-recession model through Bayesian pooling approach.
  ...
  cat('"Recession_h/tgrid.txt"', file =file.Pred3, sep="\n")
  cat(ngrid, file =file.Pred3,sep="\n", append = TRUE)
  cat("1", file = file.Pred3, append = TRUE,sep="\n")                                   #!!! n of spaghetti
  cat(".false.", file = file.Pred3, append = TRUE,sep="\n")                             #!!! Propagate parametric uncertainty?
  cat(".false.", file = file.Pred3, append = TRUE,sep="\n")                             #!!! Propagate remnant uncertainty for each output variable? (size nY)
  ...
  cat("'Recession_h/tgrid.txt'", file =file.Pred4, sep="\n")
  cat(ngrid, file =file.Pred4,sep="\n", append = TRUE)
  cat("1", file = file.Pred4, append = TRUE,sep="\n")                                   #!!! n of spaghetti
  cat(".true.", file = file.Pred4, append = TRUE,sep="\n")                              #!!! Propagate parametric uncertainty?
  cat(".false.", file = file.Pred4, append = TRUE,sep="\n")                             #!!! Propagate remnant uncertainty for each output variable? (size nY)
  ...
  cat("'Recession_h/tgrid.txt'", file =file.Pred5,sep="\n")
  cat(ngrid, file =file.Pred5, append = TRUE, sep="\n")
  cat("1", file = file.Pred5, append = TRUE,sep="\n")                                  #!!! n of spaghetti
  cat(".true.", file = file.Pred5, append = TRUE,sep="\n")                             #!!! Propagate parametric uncertainty?
  cat(".true.", file = file.Pred5, append = TRUE,sep="\n")                             #!!! Propagate remnant uncertainty for each output variable? (size nY)
...
##########################################################################################################################
  ...
  # options to be removed in future:
  limits.Y.alpha         = c(0, 1000, 200)                     # Limits for the alpha (initial stage) plot.
  limits.Y.lambda        = c(0, 40, 10)                        # Limits for the lambda (recession rate) plot.
  stage.limits           = c(-150, 50, 50)                     # limits for recession stage is in "cm" !!!
  limits.y               = stage.limits
...
")
  ...
  #####################
       ...
       # Saving selected options to txt file:
       file.options = paste0(dir.segm.recessions.param[[param]] ,"/options_recession_segmentation.txt")
       message(paste0("- Saving user-specified options to file: "))
       print(file.options)
       cat(paste0("stage.scale.shift = ", stage.scale.shift),                                file = file.options, append = FALSE, sep="\n")
       cat(paste0("recession model = ",   rec.model[iter.model]),                            file = file.options, append = TRUE, sep="\n")
       cat(paste0("Prior mean for the segments means = ",   prior.mu.rec.segment[1], 
                  "(", prior.mu.rec.segment[2],",", prior.mu.rec.segment[3],")"),            file = file.options, append = TRUE, sep="\n")
       cat(paste0("Prior for structural uncertainty of the segments = ",gamma.prior.rec[1], 
                  "(", gamma.prior.rec[2], ",",  gamma.prior.rec[3],")"),                    file = file.options, append = TRUE, sep="\n")
       cat(paste0("Maximum number of segments: ", nSmax.rec),                                file = file.options, append = TRUE, sep="\n")
       cat(paste0("Criterion for the choice of optimal segmentation: ", criterion.rec),      file = file.options, append = TRUE, sep="\n")
       cat(paste0("Number of cycles of mcmc during segmentation: ", Ncycle.rec.segment),     file = file.options, append = TRUE, sep="\n")
       cat(paste0("Number of mcmc per cycle during segmentation: ", Nmcmc.rec.segment),      file = file.options, append = TRUE, sep="\n")
       cat(paste0("Fraction of mcmc that is burned: ", Nburn.rec.segment),                   file = file.options, append = TRUE, sep="\n")
       cat(paste0("Minimum time between two shifts: ", tmin.rec),                            file = file.options, append = TRUE, sep="\n")
       cat(paste0(" Shift time adjustment: ", shift.time.adjustment.type.rec),               file = file.options, append = TRUE, sep="\n")
       cat(paste0(" perform comparison vs gaugings segmentation: ", plot.b.from.gaugings),   file = file.options, append = TRUE, sep="\n")
       cat(paste0(" param.var.model: ", param.var.model),                                    file = file.options, append = TRUE, sep="\n")
       cat(paste0(" save.all.results: ", save.all.results),                                  file = file.options, append = TRUE, sep="\n")
       cat(paste0(" plot.gamma.uncertainty: ", plot.gamma.uncertainty),                      file = file.options, append = TRUE, sep="\n")
       cat(paste0(" plot.recession.uncert: ", plot.recession.uncert),                        file = file.options, append = TRUE, sep="\n")
       cat(paste0(" segm_all_parameters: ", segm_all_parameters),                            file = file.options, append = TRUE, sep="\n")
       cat(paste0(" plot.dates: ", plot.dates),                                              file = file.options, append = TRUE, sep="\n")
       ...
       ################################################################################################################
       # initialisation of criteria for optimal segmentation:  
       ...
       N = length(data.segm.rec.BaM$h) # Number of observations to be segmented.
       end.seg = FALSE # flag for ending segmentation procedure.
       i = 1 # initialise i, number of segments
  ...
  criteria.df =  data.frame(BIC = BIC, AIC = AIC, HQC = HQC, DIC = DIC, x = seq(1, nS, 1), 
                            BICmin = BICmin, AICmin=AICmin, HQCmin = HQCmin, DICmin = DICmin)
  # Plot the model selection criteria by increasing number of segmentd nS and the minimum values:
  BICplot  = model.selection.plot(criteria.df, 
                                  dir.segm.recessions.param[[param]],  
                                  seg.iter =1)
  # select the nS which yielded the minimum value of user-selected criterion:
  ...
  # fix directory of the chosen optimal segmentation:
  ...
  mu.s             = as.numeric(Residuals[,5])   # get the segments means "mu".
  ...
  plot1 = plot.segmentation.results.rec(dir.results          = dir.segm.recessions.param[[param]],
                                        ...
                                        y.name               = Y.name,
                                        limits.Y             = limits.Y,
                                        error.type           = 3,       # hardcoded this setting for now !!
                                        plot.axis.x          = TRUE,
                                        plot.axis.y          = TRUE,
                                        ...
                                        plot.pdf             = TRUE,
                                        points.size          = 1,
                                        plot.file.name       = paste0("Segmentation_", param.var.model[param]))
  # save into files:
...
###########################################################################################
  ...
  if (Sys.info()['sysname'] == "Linux"){
    system2("./BaM", stdout =NULL, stderr = NULL)
  } else if (Sys.info()['sysname'] == "Windows"){
