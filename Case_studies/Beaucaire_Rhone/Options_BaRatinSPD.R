...
# General options:
################################################################
name.folder.results.SPD  =  "SPD_b1_b2"
#Please, read Mansanarez et al. 2019 for understanding the types of change:
...
n.parvar                 = 2                         # number of dynamic parameters
isVar                    = c(T,F,F, T,F,F)           # [T/F] Define which RC parameters are varying (b1,a1,c1, b2,a2,c2, ...)
...
b.prior                  = c(-2, 2)                  # mean for parameters "b" for the oldest period 
st_b.prior               = c(0.5, 0.3)                 # stdev for parameters "b" for the oldest period
...
show.activation.stages   = TRUE                      # [TRUE/FALSE] If you want to plot the activation stages of the lowest controls (max two controls). 
ylim.log.wind            = c(grid_RC.ylim.log[1], grid_RC.ylim.log[2])  # define Q limits for plots in log scale.
